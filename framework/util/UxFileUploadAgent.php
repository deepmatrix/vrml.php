        ...
        foreach($_FILES as $field => $file)
        {
            ...
            if(is_array($_FILES[$field]['name']))
            {
                ...
                foreach($keys as $key)
                {
                    ...
                    //获取扩展名
                    $fileext = UxFileHelper::getFileType($_FILES[$field]['tmp_name'][$key]);
                    if(is_array($fileext) || $fileext == null)
                    {
                        $fileext = UxFileHelper::getFileSuffix($_FILES[$field]['name'][$key]);
                    }
                    ...
                    {
	                    //修改图片状态值
	                    $fileInfo[$key]['name']    = UxTimeHelper::getDateTime('Ymdhis').mt_rand(100,999).'.'.$fileext;
	                    ...
	                    if(is_uploaded_file($_FILES[$field]['tmp_name'][$key]))
	                    {
	                        if(move_uploaded_file($_FILES[$field]['tmp_name'][$key],$this->dir.$fileInfo[$key]['name']))
	                        {
	                            ...
	                            {
	                                UxFileHelper::unlink($_FILES[$field]['tmp_name'][$key]);
	                            }
	                            $fileInfo[$key]['flag'] = 1;
	                        }
	                    }
            ...
            else
            {
                ...
                //获取扩展名
                $fileext = UxFileHelper::getFileType($_FILES[$field]['tmp_name']);
                if(is_array($fileext) || $fileext == null)
                {
                    $fileext = UxFileHelper::getFileSuffix($_FILES[$field]['name']);
                }
                ...
                {
	                //修改图片状态值
	                $fileInfo[0]['name']    = UxTimeHelper::getDateTime('Ymdhis').mt_rand(100,999).'.'.$fileext;
	                ...
	                if(is_uploaded_file($_FILES[$field]['tmp_name']))
	                {
	                    if(move_uploaded_file($_FILES[$field]['tmp_name'],$this->dir.$fileInfo[0]['name']))
	                    {
	                        ...
	                        {
	                            UxFileHelper::unlink($_FILES[$field]['tmp_name']);
	                        }
	                        $fileInfo[0]['flag'] = 1;
	                    }
	                }
