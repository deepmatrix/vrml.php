...
/**
 * 来源于 ThinkPHP Model模型类，基于uxf的逻辑进行改造
...
class UxTodel {
    ...
    public function __construct() {
        ...
        // 设置表前缀
        if(!$this->tablePrefix) {
            $this->tablePrefix = isset(Ux::$app->config['db']['tablePre']) ? Ux::$app->config['db']['tablePre'] : '';
         }
        ...
        $this->db(0, empty($this->connection) ? 'default' : $this->connection);
    }
    ...
    protected function _checkTableInfo() {
        ...
        if (empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if (Ux::$app->conf->dbFieldsCache) {
                $db = $this->dbName ? $this->dbName : 'default';
                $fields_cache_file = Ux::$app->getRuntimePath() . '_fields/' . strtolower($db . '.' . $this->name);
                $cacheObj = new UxCache();
                $fields = $cacheObj->get('_fields_' . strtolower($db . '.' . $this->name));
                if ($fields) {
                    $version = Ux::$app->config['tdb']['default']['_version'];
    ...
    public function flush() {
        ...
        $this->fields['_version'] = $this->db->getVersion();

        // 2008-3-7 增加缓存开关控制
        if (Ux::$app->config['dbFieldsCache']) {// 永久缓存数据表信息
            $db = $this->dbName;
            $cacheObj = new UxCache();
            $cacheObj->set('_fields_' . strtolower($db . '.' . $this->name), $this->fields);
        }
    }
    ...
    public function switchModel($type, $vars = array()) {
        ...
        if (!class_exists($class))
            throw new UxException($class . '_MODEL_NOT_EXIST_', 500);
        // 实例化扩展模型
    ...
    public function __call($method, $args) {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            throw new UxException(__CLASS__ . ':' . $method . '_METHOD_NOT_EXIST_', 500);
            return;
        }
    }
    ...
    public function db($linkNum = '', $config = '', $params = array()) {
        ...
        if (!isset($_db[$linkNum]) || (isset($_db[$linkNum]) && $config && $_linkNum[$linkNum] != $config)) {
            ...
            if (!empty($config) && is_string($config) && false === strpos($config, '/')) { // 支持读取配置参数
                //$config = isset(Ux::$app->config['tdb'][$config]) ? Ux::$app->config['tdb'][$config] : '';
                $config = 'default';
            }
            $_db[$linkNum] = UxDb::getInstance($config);
        } elseif (NULL === $config) {
    ...
    public function data($data = '') {
        ...
        if (is_object($data)) {
            ...
            throw new UxException('_DATA_TYPE_INVALID_', 500);
        }
        $this->data = $data;
        return $this;
    }
            ...
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        if(!class_exists($mappingClass)) {
                            throw new UxException($mappingClass . ' Class not existed.', 500);
                        }
                        //TODO:没有进行全局保存
                        $model  =  new $mappingClass();
