...
class UxFormValidator extends UxCommonValidateHelper
{
	private $rules = array();
	...
	function run()
	{
		...
		foreach($this->fieldData as $key => $value)
		{
			...
			{
				//获取要验证的数据
				$this->fieldData[$key][$k]['postdate'] = UxHttpRequest::get($this->fieldData[$key][$k]['name']);
				//发现有误不继续验证
				if($this->errorflag)
				{
					continue;
		...
		if ($this->errorflag)
		{
			Ux::$app->setRenderData(array('validate'=>$this->validate));
		}
