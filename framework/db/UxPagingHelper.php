...
class UxPagingHelper {
    ...
    private $pagesize; //每页的条数
    private $firstpage; //第一页
    private $lastpage; //最后一页
    private $pagelength; //要展示的页面数
    ...
    public function setSql($sql) {
        ...
        if (strpos($sql, 'GROUP BY') === false) {
            ...
            $count = $this->dbo->getAll("select count(*) as total " . $endstr);
        } else {
            $count = $this->dbo->getAll("select count(*) as total from (" . $sql . ") as IPaging");
        }
    ...
    public function getPage($page) {
        ...
        if ($this->totalpage > 0) {
            ...
            return $this->dbo->getAll($this->sql . " limit " . ($this->index - 1) * $this->pagesize . "," . ($this->pagesize));
        }
        else
            return array();
    }
