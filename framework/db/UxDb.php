...
class UxDb {
    ...
    public static function getInstance($config = '') {
        ...
        $instance = new UxDb();
        self::$instance = $instance->factory($config);
        
        return self::$instance;
    }

    /**
     * 加载数据库 支持配置文件中连接名如'default'或者直接传DSN
     * @access public
     * @param string $str_db_config 数据库配置信息 'default'等
     * @return string
     */
    public function factory($str_db_config = '') {
        if(!isset(Ux::$app->config['tdb']) || !is_string($str_db_config))
            throw new UxException('_NO_DB_CONFIG_', 500);
        
        $conf = Ux::$app->config['tdb'];
        
        if (empty($str_db_config)) {
            $db_config = $conf['default']['dsn'];
        } else {
            if (isset($conf[$str_db_config]) && isset($conf[$str_db_config]['dsn'])) {
                //若在tdb中未定义，则认为是直接传了dsn字符串
                $db_config = $conf[$str_db_config]['dsn'];
            }
        }

        // 读取数据库配置
        $db_config = $this->_parseConfig($db_config);
        
        if (empty($db_config['dbms']))
            throw new UxException('_NO_DB_CONFIG_', 500);
        
        $db_config['charset'] = isset($db_config['charset'])
                             ? $db_config['charset']
                             : 'utf8';
        $db_config['tablePrefix'] = isset($conf[$str_db_config]['tablePrefix'])
                             ? $conf[$str_db_config]['tablePrefix']
                             : '';
        
        // 数据库类型
        $this->dbType = ucwords(strtolower($db_config['dbms']));

        $class = 'UxDb' . $this->dbType;
        // 检查驱动类
        if (class_exists($class)) {
            ...
            // 类没有定义
            throw new UxException('_NO_DB_DRIVER_' . ': ' . $class, 500);
        }
        return $db;
    }
     ...
     * 分析数据库配置信息，DSN格式
     * @access private
     * @param string $db_config 数据库配置信息
     * @return string
     */
     ...
     * @return boolean
     */
    private function _validateDsnConfig($config){
        if(!is_array($config))
            return false;
        
        $dsn_params = array('type', 'user', 'pwd', 'host', 'port', 'name', 'dsn');
        
        if(count(array_intersect_key(array_flip($dsn_params), $config)) == count($dsn_params))
            return true;
        else
            return false;
    }
    ...
    protected function multiConnect($master = false) {
        ...
        $db_config = array(
            ...
            'charset'=> isset($_config['charset'][$r]) ? $_config['charset'][$r] : 'utf8',
    ...
    public function parseDSN($dsnStr) {
        if (empty($dsnStr) || !is_string($dsnStr)) {
            return false;
        }
    ...
    protected function parseWhereItem($key, $val) {
        ...
        if (is_array($val)) {
            ...
            //对字符串类型字段采用模糊匹配
            if (isset(Ux::$app->conf->dbLikeFields) && preg_match('/(' . isset(Ux::$app->conf->dbLikeFields) . ')/i', $key)) {
                $val = '%' . $val . '%';
    ...
    protected function parseJoin($join) {
        ...
        $joinStr = preg_replace("/__([A-Z_-]+)__/esU", $this->config['tablePrefix'] . ".strtolower('$1')", $joinStr);
        return $joinStr;
    }
    ...
    public function getVersion(){
        return isset($this->config['_version']) ? $this->config['_version'] : '0.1';
    }
