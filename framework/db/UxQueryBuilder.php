...
class UxQueryBuilder {
    ...
    public function __construct($name) {
        $this->tablePre = isset(Ux::$app->config['db']['tablePre']) ? Ux::$app->config['db']['tablePre'] : '';
        $this->table = $name;
    ...
    public function __set($name, $value) {
        switch ($name) {
            ...
            case 'fields':$this->sql['fields'] = $value;
                break;
            case 'where':$this->setWhere($value);
                break;
            ...
            case 'order':$this->sql['order'] = 'order by ' . $value;
                break;
            case 'limit':$value == 'all' ? '' : ($this->sql['limit'] = 'limit ' . $value);
                break;
    ...
    public function find() {
        if (is_int($this->page)) {
            ...
            $this->paging = new UxPagingHelper($sql, $pagesize, $pagelength);
