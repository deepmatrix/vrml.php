 ...
 * @note
 */
class UxFileLogger implements IUxLogger {
    ...
    public function write($logs = array()) {
        if (empty($logs)){
            throw new UxException('the $logs cann\'t be empty.');
        }
        if(!is_array($logs)) $logs = array($logs);

        if ($this->path == '') {
            throw new UxException('the file path is undefined');
        }
