 ...
 * @version 0.6
 */
class UxCache implements IUxCache {
    ...
    public function __construct($cacheType = '') {
        $this->cacheType = $cacheType 
                    ? $cacheType 
                    : (isset(Ux::$app->config['cache']['type']) 
                        ? Ux::$app->config['cache']['type'] 
                        : $this->cacheType);
        $this->expire = isset(Ux::$app->config['cache']['expire']) 
                    ? Ux::$app->config['cache']['expire'] 
                    : $this->expire;
        $this->timeout = isset(Ux::$app->config['cache']['timeout']) 
                    ? Ux::$app->config['cache']['timeout'] 
                    : $this->timeout;

        //当前系统支持的缓存类型adapter
        $this->cache = self::factory($this->cacheType);
    }
    ...
    public static function factory($adapter, $options = null) {
        ...
        if (in_array($adapter, array('file', 'db', 'apc', 'xcache', 'eaccelerator', 'mem'))) {
            $adapter = 'Ux' . ucfirst($adapter) . 'CacheAdapter';
            return new $adapter();
        }
