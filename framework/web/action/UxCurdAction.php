<?php
/**
 * @brief 完成用户的自动增加删除修改功能DURD类，实现系统的自动增删改查
 * @author webning
 * @date 2010-12-23
 * @version 0.6
 */
class UxCurdAction extends UxAbstractAction {
            ...
            switch ($action) {
                ...
                case 'upd': {
                        ...
                        if ($validate != null) {
                            ...
                            $formValidate = new UxFormValidator($validate);

                            $data = $formValidate->run();
                        }

                        $model = new UxDbModel($modelName);
                        ...
                        if (isset($data) && $data !== null) {
                            ...
                            if ($action = 'add')
                                ...
                                $flag = $model->upd("$key = '" . UxHttpRequest::get($key) . "'");
                        }
                        if (isset($flag) && $flag) {
                ...
                case 'del': {
                        $model = new UxDbModel($modelName);
                        $flag = $model->del("$key = '" . UxHttpRequest::get($key) . "'");
                        if ($flag) {
                            $_GET['action'] = $actions['success'];
                        } else {
                            $_GET['action'] = $actions['fail'];
                ...
                case 'get': {
                        $model = new UxDbModel($modelName);
                        $rs = $model->getOne("$key = '" . UxHttpRequest::get($key) . "'");
                        echo JSON::encode($rs);
                        return false;
                    }
            }
            ...
            if ($modelName !== null) {
                ...
                if (is_array($actions)) {
                    ...
                    if ($current !== null) {
                        ...
                        throw new UxException('class ' . get_class($controller) . ' have curd error');
                    }
                }
            }
        }
