...
class UxHttpCookie
{
	...
	public static function set($name,$value='',$time='3600',$path='/',$domain=null)
	{
		...
		setCookie(self::$pre.'safecode',  UxEnryptor::encode( self::cookieId() , self::getKey()  )       ,$time,$path,$domain);
		if(is_array($value) || is_object($value)) $value=serialize($value);
		$value = UxEnryptor::encode($value , self::getKey() );
	...
	public static function get($name)
	{
		...
		if($is_checked == 1)
		{
			...
			{
				$cookie= UxEnryptor::decode($_COOKIE[self::$pre.$name],self::getKey());
	...
	private static function checkSafe()
	{
		...
		{
			if( self::cookieId() == UxEnryptor::decode($_COOKIE[self::$pre.'safecode'] , self::getKey())    )
			{
				return 1;
			}
			else
	...
	{
		$encryptKey = isset(Ux::$app->config['encryptKey']) ? Ux::$app->config['encryptKey'] : self::$defaultKey;
